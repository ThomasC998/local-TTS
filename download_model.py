...
Two different checkpoints, because the two backends genuinely need different
files -- see MODELS.md for the whole story, and for how to point this at a
different model:

    macOS    an MLX INT8 artifact, quantized for Apple Silicon    ~3.5 GB
    Windows  the original Hugging Face checkpoint, for CUDA       ~7 GB
...
SOURCES: dict[str, dict[str, object]] = {
    ...
    "torch": {
        ...
        "size": "~7 GB",
        "what": "original Hugging Face checkpoint, for CUDA",
        # The repository also carries training-only files and alternative
        # formats. These are what inference reads, and skipping the rest is
        # several gigabytes of download that would never be opened.
        "allow": [
            "config.json",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--backend", choices=sorted(SOURCES), default=None)
    ...
    configured = os.getenv("BREEZE_MODEL") if backend == backend_name() else None
    dest = Path(args.dest or configured or str(source["dest"]))
    if not dest.is_absolute():
        dest = PROJECT / dest

    print(f"Breeze TTS 2 model — {backend} backend")
    ...
    if status == 0:
        print(f"\nDone. {dest} is ready.")
        if dest.name != str(SOURCES[backend]["dest"]):
